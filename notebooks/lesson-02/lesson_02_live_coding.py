...
for col in num_cols_raw:
    # TODO(LIVE): calcolare Q1, Q3, IQR e le fence inferiore/superiore; contare gli outlier
    # Hint: usa df_clean[col].quantile(); fence = Q1 ∓ 1.5 * IQR; confronta ogni valore con le fence
    pass
...
# TODO(LIVE): creare la feature binaria balance_is_zero da Balance
# Hint: usa un confronto booleano su df_clean["Balance"] e converti con .astype(int)
...
# TODO(LIVE): eseguire lo split train/validation/test (60/20/20) stratificato sul target
# Hint: usa due chiamate a train_test_split — prima separa train (60%) da tmp (40%),
#       poi dividi tmp a metà in val e test; usa stratify=y e random_state=SEED
...
# TODO(LIVE): istanziare un OneHotEncoder e fittarlo su X_train[cat_cols]
# Hint: usa handle_unknown="ignore" e sparse_output=False; salva l'oggetto in ohe_demo
...
# TODO(LIVE): Definire num_pipeline e cat_pipeline per le feature numeriche e categoriche
# rispettivamente, poi costruire il ColumnTransformer completo. Hint: num_pipeline utilizza
# SimpleImputer e StandardScaler; cat_pipeline utilizza SimpleImputer e OneHotEncoder.
# Costruire il ColumnTransformer, fittarlo su X_train e trasformare i tre split
# Hint: usa transformers=[("num", num_pipeline, num_cols), ("cat", cat_pipeline, cat_cols)];
#       chiama .fit(X_train) poi .transform() su train, val e test; salva in preprocessor
